...
def main():
    ...
    for date_str, rows in by_date.items():
        ...
        # Any Savant HR-hitter whose normalized name never matched a pending row
        # either wasn't in the tracked roster that day, OR is a real matching miss.
        unmatched_savant = [h['name'] for norm, h in hr_map.items() if norm not in matched_names]
        if unmatched_savant:
