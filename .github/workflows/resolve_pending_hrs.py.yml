...
def main():
    ...
    skipped_dates = []
    errored_dates = []
    # FIXED 7/24/2026 — process oldest date first. Previously used whatever
    # order the dict happened to iterate in (insertion order from the raw
    # query), meaning newer dates could get processed before older ones with
    # no guarantee the backlog would ever actually shrink.
    for date_str in sorted(by_date.keys()):
        rows = by_date[date_str]
        # FIXED 7/24/2026 — CRITICAL: this entire block previously had ZERO
        # error handling. If all_games_final() (or anything else below) threw
        # for ANY reason — an MLB API timeout, an unexpected response shape,
        # anything — the WHOLE SCRIPT crashed right there, and every date that
        # would have been processed after it in the loop never got touched
        # that run. Repeat that pattern for a week and you get exactly what
        # happened: a growing, never-clearing backlog across multiple dates,
        # because one bad date silently killed all the good ones after it.
        # Now a failure on one date is caught, logged, and the run continues.
        try:
            ...
            print(f"  ✗ ERROR processing {date_str}: {e} — moving on to next date, this date will retry next run")
            errored_dates.append(date_str)
            continue

    tail = f", {len(skipped_dates)} date(s) skipped (games not final yet)" if skipped_dates else ""
    tail += f", {len(errored_dates)} date(s) errored (see above, will retry next run)" if errored_dates else ""
