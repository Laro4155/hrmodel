...
def get_starting_pitchers(boxscore):
    """
    Returns dict: {'away': 'Pitcher Name', 'home': 'Pitcher Name'}
    The starting pitcher is the first pitcher listed in each team's pitchers array.
    The AWAY pitcher faces HOME batters, and the HOME pitcher faces AWAY batters.
    """
    ...
    for side in ('away', 'home'):
        ...
        if pitchers:
            # First pitcher in the array is the starter
            starter_id = pitchers[0]
            p = players.get(f'ID{starter_id}') or players.get(str(starter_id))
            if p:
                starters[side] = p.get('person', {}).get('fullName')
    return starters
    ...
    for p in hr_plays:
        ...
        if norm not in hr_map:
            hr_map[norm] = {'name': name, 'count': 0, 'ev': 0, 'angle': 0, 'dist': 0,
                           'actual_pitcher': None, 'pitcher_role': None}
        hr_map[norm]['count'] += 1
        ...
        if ev > hr_map[norm]['ev']:
            ...
            # Capture actual pitcher who gave up the HR
            pitcher = p.get('pitcher_name', '')
            if pitcher:
                hr_map[norm]['actual_pitcher'] = pitcher
                # Inning 1-5 = starter territory, 6+ = likely reliever/bulk
                try:
                    inning = int(p.get('inning', 1))
                    hr_map[norm]['pitcher_role'] = 'starter' if inning <= 5 else 'reliever'
                except (ValueError, TypeError):
                    hr_map[norm]['pitcher_role'] = 'unknown'
...
def run_sweep(date_str):
    ...
    for game in final_games:
        ...
        # Get starting pitchers — away starter faces home batters, home starter faces away batters
        starters = get_starting_pitchers(boxscore)
        # opposing_pitcher[side] = the pitcher that side's batters faced
        opposing_pitcher = {
            'away': starters.get('home'),   # away batters face the home starter
            'home': starters.get('away'),   # home batters face the away starter
        }
        ...
        for side in ('away', 'home'):
            ...
            for pid in player_ids:
                ...
                if hr_data:
                    ...
                    actual_pitcher = hr_data.get('actual_pitcher')
                    pitcher_role = hr_data.get('pitcher_role')
                else:
                    result = 'No HR'
                    ev = angle = dist = grade = None
                    actual_pitcher = None
                    # For No HR rows, pitcher_role is always 'starter' since the
                    # pre-game starter is who the prop was priced against
                ...
                # side is 'away' or 'home' from the loop — direct derivation, no guessing
                home_away = side  # 'away' or 'home'
                ...
                all_rows.append({
                    ...
                    'team': team_name,
                    'home_away': home_away,
                    'pitcher_name': opposing_pitcher.get(side),  # pre-game starter (prop pricing context)
                    'actual_pitcher': actual_pitcher,            # pitcher who actually gave up the HR
                    'pitcher_role': pitcher_role,               # 'starter', 'reliever', or None
                    'result': result,
                    'hit_grade': grade,
                    'ev': ev,
                    'angle': angle,
